    ...
    for rec in records:
        ...
        response.append(
            DocumentDetailResponse(
                ...
                classification_reasoning=rec.classification_reasoning,
            )
        )
    return response
...
@router.post(
    "/documents/{doc_id}/rerun-classification",
    response_model=DocumentDetailResponse,
)
async def rerun_classification(
    doc_id: int,
    current_user: User = Depends(get_current_user),
    session: Session = Depends(get_session),
):
    """CA Admin endpoint: Re-run classification and keep the document in review."""
    if current_user.role != UserRole.CA_ADMIN:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="CA Admin access required"
        )
    ...
    try:
        doc = rerun_document_classification(doc, session)
